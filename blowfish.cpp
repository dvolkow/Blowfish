...
template<typename T>
void debug_(const T & value, std::string const & valname)
{
    std::cerr << valname + " = " << value << "\n";
}
...
void Blowfish::SetKey(const char *key, size_t byte_length) 
{
...

    {
        ...
        debug_(buffer_length, "buffer_length");
...
std::vector<char> Blowfish::Encrypt(const std::vector<char> &src) const 
{
    ...
    for (uint i = 0; i < dst.size() / sizeof(uint64_t); ++i) 
    {
